...
fourcc = 0x00000021
fps = 30.0
video  = cv2.VideoWriter(os.path.join(os.path.dirname(__file__), 'output_inner.mp4'), fourcc, fps, (DISPLAY_WIDTH, DISPLAY_HEIGHT))

def load_texture():
    global textureIDs
    filepath = os.path.join(os.path.dirname(__file__), 'data/test4.jpg')
    img = Image.open(filepath)
    width, hight = img.size
    gl.glBindTexture(gl.GL_TEXTURE_2D, textureIDs[0])
    gl.glTexImage2D(gl.GL_TEXTURE_2D, 0, gl.GL_RGB, width, hight, 0, gl.GL_RGB, gl.GL_UNSIGNED_BYTE, img.tobytes())
...
def on_draw():
    ...
    gl.gluLookAt(0, 0, 0, 0, 0, -1, 0, 11, 0)
   ...
   #     (0.1, 4)
    ...
    gl.glTranslatef(0.5 / image_data.width, 0.5 / image_data.height, 1)
    ...
    gl.glMatrixMode(gl.GL_MODELVIEW)
    gl.glRotatef(70.0,1.0,0.0,0.0)  
    #gl.glTranslatef(0,3,-2)
